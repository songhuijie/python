...
proxy_ip_own = '118.172.201.89:38156'
proxies = {
...
def getData():
    ...
    cur = conn2db.cursor()
    # sql = "SELECT `id`,`video_cover` FROM `xvideo`  where  video_cover_handle = 1  order By id desc limit %s,%s" % (
    #    offset, limit)
    sql = "SELECT `id`,`video_cover` FROM `xvideo`  where  video_cover_handle = 1  order By id desc"
...
def saveData2(id):
    ...
    now_time = 1
    cur = conn2db.cursor()
    sql = "UPDATE  `xvideo_month`  set `video_cover_handle` = '%s' where id = '%s'"% (now_time,id)
    try:
...
def saveData(data):
    ...
    cur = conn2db.cursor()
    sql = "UPDATE  `xvideo`  set `video_cover_handle` = 0 where id in (%s)"% (data)
    # exit()
...
def handleContent(data,content_array):
    ...
    try:
        ...
        content_array.append(title.get('content'))
...
if __name__ == '__main__':
    ...
    for value in data:
        if value[1]:
            if value[1].split(':')[0] == 'https':
                own_data.append(value[0])
            else:
                continue
        else:
            continue

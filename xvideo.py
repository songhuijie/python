...
if __name__ == '__main__':
    ...
    for value in cate_names:
        ...
        print('cate_id为:'+ str(value[1]) + '开始抓取100条数据')
        page = 0
        content = []
        # html = reqPage(search_url + value[0] + '&p=' + str(page))
        # content = handleSoup(BeautifulSoup(html, 'html5lib'), content,value[1])
        ...
        print('开始抓取100个详情页面')
        for val in content:
            result = handleContent(reqPage(val[0]), val)
            time.sleep(5)
            final_array.append(result)

        print('详情页面抓取完毕')
        saveData(final_array)
        print('存入数据库完毕')
        time.sleep(600)
        print('抓取等待10分钟')
        print('开始下个循环')

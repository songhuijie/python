...
proxy_ip_own = '91.229.240.93:31761'
proxies = {
...
def getData():
    ...
    cur = conn2db.cursor()
    sql = "SELECT `id`,`video_access_address` FROM `xvideo_week`  where insert_time = 0 and cate_id=60 order By id desc limit %s,%s" % (
        offset, limit)
    cur.execute(sql)
...
def saveData2(id):
    ...
    now_time = 1
    cur = conn2db.cursor()
    sql = "UPDATE  `xvideo_week`  set `insert_time` = '%s' where id = '%s'"% (now_time,id)
    try:
...
def saveData(data):
    ...
    cur = conn2db.cursor()
    sql = "UPDATE  `xvideo_week`  set `video_title` = '%s',`video_duration` = '%s',`video_cover` = '%s',`video_urlLow` = '%s',`video_urlhigh` = '%s',`video_hls` = '%s',`video_hls_url` = '%s',`views` = '%s',`insert_time` = '%s',`video_tag` = '%s' where id = '%s'"% (data[2],data[5],data[8],data[3],data[4],data[7],data[6],data[1],now_time,data[9],data[0])
...
def getProxydata():
    ...
    myobj = {'key': '20190829153652708','getnum':'30','isp':'0','anonymoustype':'3','area':'2','order':'2','filter':'1','formats':'2','proxytype':'1'}
    num = 0
    # 一个页面最多尝试请求三次
    while num < 3:
        # 请求页面信息,添加请求异常处理，防止某个页面请求失败导致整个抓取结束，
        try:
            if url:
                ...
                # req = requests.get(url, headers=headers,cookies = cookies ,proxies=proxies)
                req = requests.post(url, headers=headers,timeout=5,data = myobj)
                # req = requests.get(url, headers=headers,cookies = cookies) 带cookies
                if req.status_code == 200:
                    # 返回BeautifulSoup对象
                    return req.text
